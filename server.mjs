...
//   connectionLimit: 100, // Maximum number of connection in pool
...
// Auto refresh database connection every 24 hours
setInterval(async () => {
...
}, 24 * 60 * 60 * 1000); // Execute every 24 hours


async function getData() {
  // Use pool to get the connection
  const [tearSheetData] = await pool.execute('SELECT * FROM tear_sheet');
  const [eventsData] = await pool.execute('SELECT * FROM ts_events');
...
// It will bring the server/sheet down :(
...
async function updateDB() {
  ...
  await pool.execute('DELETE FROM tear_sheet');
  //insert new data
  ...
  await pool.query(insertQuery, [tearSheetValues]);

  // complete dataset in smaller batches
  const BATCH_SIZE = 10;

  await pool.execute('DELETE FROM ts_events');
  ...
  for (let i = 0; i < tearSheet.length; i += BATCH_SIZE) {
    ...
    await pool.query(insertQuery, [eventValues]);
  }
}
